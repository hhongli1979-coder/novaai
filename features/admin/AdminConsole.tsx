...
const AdminConsole: React.FC = () => {
  ...
  const [showVercelOrg, setShowVercelOrg] = useState(false);
  const [showVercelProject, setShowVercelProject] = useState(false);
  ...
  const handleProvisionSchema = () => {
    ...
    setTimeout(() => {
      ...
      alert("Nova OS Schema successfully provisioned on Neon Cluster: ep-noisy-recipe-ag4paq9q.");
    }, 3000);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="glass-card rounded-[3rem] p-10 border border-white/5 space-y-8">
          <h3 className="text-xl font-black text-white uppercase tracking-widest flex items-center">
            <i className="fa-solid fa-toggle-on mr-3 text-indigo-400"></i>
            Module Access Control
          </h3>
          <div className="space-y-6">
            {[
              { id: 'isChatEnabled', label: 'Neural Intelligence', icon: 'fa-brain' },
              { id: 'isImageEnabled', label: 'Image Forge', icon: 'fa-wand-magic-sparkles' },
              { id: 'isVideoEnabled', label: 'Cinema Studio', icon: 'fa-clapperboard' },
        ...
        <div className="glass-card rounded-[3rem] p-10 border border-white/5 space-y-8">
          <h3 className="text-xl font-black text-white uppercase tracking-widest flex items-center">
            <i className="fa-solid fa-dna mr-3 text-indigo-400"></i>
            Neural DNA settings
          </h3>
          <div className="space-y-6">
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Global Compute Burst Budget (CR)</label>
              <input 
                type="range" 
                min="10000" 
                max="100000" 
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest text-indigo-400">Primary Domain Infrastructure</label>
              <div className="relative">
                <input 
                  type="text"
                  value={settings.baseDomain}
                  onChange={(e) => updateSettings({ baseDomain: e.target.value })}
                  className="w-full bg-slate-950 border border-white/10 rounded-2xl p-4 text-sm font-bold text-white outline-none focus:ring-2 focus:ring-indigo-500/50"
                />
                <div className="absolute right-4 top-1/2 -translate-y-1/2 text-emerald-500">
                  <i className="fa-solid fa-globe"></i>
                </div>
              </div>
              <p className="text-[9px] text-slate-500 uppercase font-black">Subdomain routing enabled for *.{settings.baseDomain}</p>
            </div>
            
            <div className="space-y-3">
              <div className="flex items-center justify-between p-4 bg-slate-900 rounded-2xl border border-white/5">
                <div className="flex-1">
                   <p className="text-xs font-black text-indigo-400 uppercase tracking-widest mb-1">AI Gateway API Switch</p>
                  <p className="text-[9px] text-slate-500 uppercase">Traffic routing: {settings.isAiGatewayEnabled ? 'Proxy active' : 'Direct link'}</p>
                </div>
                <button 
                  onClick={() => updateSettings({ isAiGatewayEnabled: !settings.isAiGatewayEnabled })}
                  className={`w-14 h-7 rounded-full transition-all relative ${ settings.isAiGatewayEnabled ? 'bg-emerald-600 shadow-[0_0_15px_#10b98144]' : 'bg-slate-700' }`}
                >
                  <div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${ settings.isAiGatewayEnabled ? 'left-8' : 'left-1' }`}></div>
                </button>
              </div>
              {settings.isAiGatewayEnabled && (
                 <input 
                  type="text"
                  placeholder="https://gateway.mdio.shop/v1"
                  value={settings.aiGatewayUrl}
                  onChange={(e) => updateSettings({ aiGatewayUrl: e.target.value })}
                  className="w-full bg-slate-950 border border-white/10 rounded-xl p-4 text-xs font-mono text-slate-300"
                />
              )}
            </div>
          </div>
        ...
        <div className="glass-card rounded-[3rem] p-10 border border-white/10 space-y-8 bg-slate-900/50 col-span-1 lg:col-span-2">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-black text-white uppercase tracking-widest flex items-center">
              <i className="fa-solid fa-cloud-arrow-up mr-3 text-indigo-400"></i>
              Vercel CI/CD Pipeline Infrastructure
            </h3>
            <div className="flex space-x-3">
              <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-black">
                <i className="fa-brands fa-vercel"></i>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex justify-between">
                VERCEL_TOKEN
                <button onClick={() => setShowVercelToken(!showVercelToken)} className="text-indigo-400 hover:text-indigo-300">
                   <i className={`fa-solid ${showVercelToken ? 'fa-eye-slash' : 'fa-eye'}`}></i>
                </button>
              </label>
              <div className="relative">
                <input 
                  ...
                  onChange={(e) => updateSettings({ vercelToken: e.target.value })}
                  className="w-full bg-slate-950 border border-white/10 rounded-2xl p-4 text-xs font-mono text-indigo-400 outline-none focus:ring-2 focus:ring-indigo-500/50"
                />
              </div>
            </div>
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex justify-between">
                VERCEL_ORG_ID
                <button onClick={() => setShowVercelOrg(!showVercelOrg)} className="text-indigo-400 hover:text-indigo-300">
                   <i className={`fa-solid ${showVercelOrg ? 'fa-eye-slash' : 'fa-eye'}`}></i>
                </button>
              </label>
              <input 
                type={showVercelOrg ? "text" : "password"}
                value={settings.vercelOrgId}
                onChange={(e) => updateSettings({ vercelOrgId: e.target.value })}
                className="w-full bg-slate-950 border border-white/10 rounded-2xl p-4 text-xs font-mono text-slate-300 outline-none focus:ring-2 focus:ring-indigo-500/50"
              />
            </div>
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex justify-between">
                VERCEL_PROJECT_ID
                <button onClick={() => setShowVercelProject(!showVercelProject)} className="text-indigo-400 hover:text-indigo-300">
                   <i className={`fa-solid ${showVercelProject ? 'fa-eye-slash' : 'fa-eye'}`}></i>
                </button>
              </label>
              <input 
                type={showVercelProject ? "text" : "password"}
                value={settings.vercelProjectId}
                onChange={(e) => updateSettings({ vercelProjectId: e.target.value })}
                className="w-full bg-slate-950 border border-white/10 rounded-2xl p-4 text-xs font-mono text-slate-300 outline-none focus:ring-2 focus:ring-indigo-500/50"
              />
            </div>
          </div>
          ...
          <div className="pt-6 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6">
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest text-center md:text-left">
              Securely mapped to Team: <span className="text-indigo-400">{showVercelOrg ? settings.vercelOrgId : '********'}</span>
            </p>
            <div className="flex space-x-4">
              <button className="px-6 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-[10px] font-black text-white uppercase tracking-widest transition-all">
          ...
          <div className="glass-card rounded-[3rem] p-10 border border-white/5 space-y-6">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest flex items-center">
                ...
                Telemetry Stream (neondb_owner@ep-noisy-recipe)
              </h4>
